...
class User
{
    ...
    public function __construct(
        ...
        public function getUserByID($id)
        {
            ...
            $req = $con->query("SELECT id,username,team,registered FROM user where id='$id' limit 1");
            if($req){
                $result = $req->fetch();
                if(isset($result['id'])){
                    return new User($result['id'],$result['username'],$result['team'], $result['registered']);
                } else {
                    return false;
                }
            } else {
        ...
        public function getUserByUsername($username)
        {
            ...
            $req = $con->query("SELECT id,username,team,registered FROM user where username='$username' limit 1");
            if($req){
                $result = $req->fetch();
                return new User($result['id'],$result['username'],$result['team'], $result['registered']);
            } else {
        ...
        public function getRegisteredUsers(){
            ...
            $usernames = $con->query("SELECT username FROM user_view WHERE id NOT IN admin AND registered=1");
        ...
        public function getUnprocessedSubmissions()
        {
            $list = array();

            foreach ($this->getSubmissions() as $result){
                if(!$result->isProcessed) $list[] = $result;
            }
            return $list;
        }
        ...
        public function confirmRegistration()
        {
            ...
            $req = $con->query("UPDATE `user` SET registered = 1 WHERE id = '$this->id'");
            if($req){
                return true;
            } else {
                return false;
