...
class Uploader
{
    public static function execInBackground($input) {
        //$cmd = "python3 scripts/FandS.py " . $input . " ". $shuffle . " " . $footprint ;
        $cmd = "python3 -m scripts.sender " . $input;
    ...
    public static function anonym()
    {
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            global $security;
            $id = $security->idGenerator();
            $fileName = "user_{$_SESSION['user']->id}_{$id}";
            rename("{$filePath}.part", "uploads/".$fileName.".zip");

            $FFileName = "F_{$fileName}";
            $SFileName = "S_{$fileName}";
            // Create blank files
            //$SFile = fopen("files/".$SFileName, "w");
            //$FFile = fopen("files/".$FFileName, "w");
            ...
            self::execInBackground($fileName);
        }
    }
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], "uploads/".$tmpfile)) {
            ...
            if($anonymisation){
                ...
                if($teamDefence->id != $teamAttack->id){
                    ...
                    $attack = new Attack();
                    $result = checkAttackJson("files/".$anonymisation->F_file, "uploads/".$tmpfile);
                    if($result==-1){
                        return false;
                                }
